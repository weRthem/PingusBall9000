...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] PlayerServerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is PlayerServerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerServerNetworkObject.Length > 0 && PlayerServerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlayerServerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlayerServerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiatePlayerServer instead, its shorter and easier to type out ;)")]
		public PlayerServerBehavior InstantiatePlayerServerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerServerNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerServerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PlayerServerBehavior>().networkObject = (PlayerServerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of PlayerServer
		/// </summary>
		/// <returns>
		/// A local instance of PlayerServerBehavior
		/// </returns>
		/// <param name="index">The index of the PlayerServer prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlayerServerBehavior InstantiatePlayerServer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (PlayerServerNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: PlayerServer. Add them at the NetworkManager prefab.");
				return null;
			}
